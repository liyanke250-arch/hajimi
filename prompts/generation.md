...
• 概念脱钩 (知识平行原则):
o 核心定义: 彩蛋内容必须是与核心实体相关的**"平行知识"或"趣味事实"，作为答题后的延伸阅读。可以在相关语境中自然使用选项词汇，但严禁直接解释其在题目中的作用或暗示其为正确答案。其目的是"拓宽知识面，而非指明解题路"**。
o 软剧透与硬剧透的区别:
  - **允许（软剧透）**: 在语境中自然使用关键词，作为知识拓展。例如："谈判中表明'绝不妥协'（brook no compromise）是常见策略。"
  - **禁止（硬剧透）**: 直接解释词汇在题目中的用法或暗示答案。例如："所以这道题选brook，因为..."
o 自我审查指令: 在生成彩蛋后，必须进行自我审查并回答："彩蛋是否直接解释了选项词在题目中的作用？是否暗示了正确答案？" 如果答案为"是"，则该彩蛋内容违规，必须重写。
